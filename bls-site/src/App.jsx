...
const photos = Array.from({ length: 38 }, (_, i) => {
  try { return new URL(`./assets/photos/photo${i + 1}.jpg`, import.meta.url).href }
  catch { return null }
}).filter(Boolean)
...
const SERVICES = [
  {
    icon: '🏗️',
    title: 'Plâtrerie',
    desc: 'Pose de placo, isolation, faux plafonds, cloisons. Finitions soignées pour des intérieurs impeccables.',
  },
  {
    icon: '🎨',
    title: 'Peinture',
    desc: 'Peinture intérieure et extérieure, préparation des supports, application professionnelle.',
  },
  {
    icon: '🔨',
    title: 'Rénovation',
    desc: 'Rénovation complète d\'appartements et maisons : sols, murs, plafonds, carrelage.',
  },
  {
    icon: '🪟',
    title: 'Menuiserie',
    desc: 'Pose de portes, fenêtres, parquet. Remplacement et installation sur mesure.',
  },
]
    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', gap: 36, alignItems: 'center' }} className="desktop-nav">
        ...
        <a href="tel:0611024833" style={{
          ...
          fontWeight: 600, letterSpacing: 0.5, transition: 'background 0.2s',
        }}
      ...
      <button onClick={() => setMenuOpen(!menuOpen)} style={{
        ...
        color: 'white', fontSize: 24, padding: 8,
      ...
      {menuOpen && (
        <div style={{
          ...
          background: 'var(--dark)', zIndex: 200,
          ...
          {links.map(l => (
            <a key={l.href} href={l.href} onClick={() => setMenuOpen(false)} style={{
              ...
              fontFamily: 'var(--font-display)', fontSize: 42, letterSpacing: 4,
            }}>{l.label}</a>
          ))}
          <a href="tel:0611024833" style={{
...
function Hero() {
  ...
  return (
    <section style={{
      ...
      {/* Content */}
      <div style={{ position: 'relative', zIndex: 1, padding: '0 60px', maxWidth: 900 }}>
        <div style={{
        ...
        }}>Estrablin · Isère · Depuis 2010</div>

        <h1 style={{
          fontFamily: 'var(--font-display)', fontSize: 'clamp(64px, 10vw, 130px)',
          color: 'white', lineHeight: 0.9, letterSpacing: 2,
        ...
        <p style={{
          color: 'rgba(255,255,255,0.7)', fontSize: 18, maxWidth: 520,
          lineHeight: 1.7, marginTop: 28, marginBottom: 40,
          opacity: loaded ? 1 : 0, transform: loaded ? 'translateY(0)' : 'translateY(20px)',
        ...
        <div style={{
          ...
          <a href="#realisations" style={{
            ...
            padding: '16px 24px', borderRadius: 8, textDecoration: 'none',
        ...
        <div style={{
          ...
          {[
            { val: '4,9/5', label: '85 avis Google' },
    ...
    <section id="services" ref={ref} style={{ padding: '100px 40px', background: 'var(--cream)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(42px, 6vw, 72px)', lineHeight: 0.95, marginBottom: 60, color: 'var(--dark)' }}>
            NOS<br /><span style={{ color: 'var(--gold)' }}>SERVICES</span>
          </h2>
        </div>
        ...
        {/* CTA bande */}
        <div style={{
          marginTop: 60, background: 'var(--dark)', borderRadius: 16, padding: '40px 48px',
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <a href="tel:0611024833" style={{
              background: 'var(--gold)', color: 'white', padding: '14px 28px',
  ...
  return (
    <section id="realisations" ref={ref} style={{ padding: '100px 40px', background: 'var(--dark)' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{
          opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(30px)',
          transition: 'all 0.7s ease', marginBottom: 48,
        }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(42px, 6vw, 72px)', lineHeight: 0.95, color: 'white' }}>
            NOS<br /><span style={{ color: 'var(--gold)' }}>RÉALISATIONS</span>
          </h2>
        </div>
        ...
        <div style={{ columns: 'auto 260px', gap: 12 }}>
          {displayPhotos.map((src, i) => (
            <div key={i} style={{
              ...
              opacity: inView ? 1 : 0,
              transition: `opacity 0.5s ease ${Math.min(i * 0.04, 0.8)}s`,
            }}
              onClick={() => setSelected(src)}
            >
              <img src={src} alt={`Réalisation BLS ${i + 1}`}
              ...
              />
              <div style={{
                position: 'absolute', inset: 0, background: 'rgba(200,137,58,0)',
              ...
              >
                <span style={{ color: 'white', fontSize: 28, opacity: 0 }}
      ...
      {selected && (
        <div onClick={() => setSelected(null)} style={{
          ...
          <button onClick={() => setSelected(null)} style={{
            position: 'absolute', top: 20, right: 20, background: 'var(--gold)',
            border: 'none', color: 'white', fontSize: 24, width: 44, height: 44,
    ...
    <section id="avis" ref={ref} style={{ padding: '100px 40px', background: 'var(--cream-2)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(42px, 6vw, 72px)', lineHeight: 0.95, color: 'var(--dark)' }}>
            AVIS<br /><span style={{ color: 'var(--gold)' }}>CLIENTS</span>
          </h2>
...
function Contact() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    setStatus('sending')
    // Formspree integration — remplacer YOUR_FORM_ID
    try {
  ...
  const inputStyle = {
    width: '100%', padding: '14px 18px', borderRadius: 8, fontSize: 15,
    border: '2px solid var(--cream-2)', background: 'white', color: 'var(--dark)',
    ...
    <section id="contact" ref={ref} style={{ padding: '100px 40px', background: 'var(--dark)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'start' }}>
        ...
        <div style={{ opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateX(-30px)', transition: 'all 0.7s ease' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(42px, 5vw, 64px)', color: 'white', lineHeight: 0.95, marginBottom: 40 }}>
            PARLONS<br /><span style={{ color: 'var(--gold)' }}>DE VOTRE<br />PROJET</span>
          </h2>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {[
              { icon: '📞', label: 'Téléphone', val: '06 11 02 48 33', href: 'tel:0611024833' },
              { icon: '💬', label: 'WhatsApp', val: 'Envoyer un message', href: 'https://wa.me/33611024833' },
              { icon: '📍', label: 'Adresse', val: '1 All. de la Divinières\n38780 Estrablin', href: 'https://maps.google.com/?q=1+All.+de+la+Divinières+38780+Estrablin' },
              { icon: '🕐', label: 'Horaires', val: 'Lun–Ven : 07h30 – 18h30\nSam–Dim : Fermé' },
            ].map((info, i) => (
              <div key={i} style={{ display: 'flex', gap: 18, alignItems: 'flex-start' }}>
                <div style={{
                  width: 44, height: 44, background: 'rgba(200,137,58,0.15)',
                  borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 20, flexShrink: 0,
                }}>{info.icon}</div>
                <div>
                  <div style={{ fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 4 }}>{info.label}</div>
                  {info.href ? (
                    <a href={info.href} target={info.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer" style={{ color: 'white', textDecoration: 'none', fontSize: 16, fontWeight: 500, whiteSpace: 'pre-line', lineHeight: 1.5 }}
                      onMouseEnter={e => e.currentTarget.style.color = 'var(--gold)'}
                      onMouseLeave={e => e.currentTarget.style.color = 'white'}
                    >{info.val}</a>
                  ) : (
                    <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 15, whiteSpace: 'pre-line', lineHeight: 1.5 }}>{info.val}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        <div style={{ opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateX(30px)', transition: 'all 0.7s ease 0.2s' }}>
          <form onSubmit={handleSubmit} style={{
            background: 'var(--dark-3)', borderRadius: 20, padding: '40px 36px',
            ...
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 24, fontStyle: 'italic' }}>
              📧 Email disponible bientôt — En attendant, appelez-nous ou utilisez WhatsApp
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <input placeholder="Téléphone" type="tel" value={form.phone}
                ...
                onBlur={e => e.target.style.borderColor = 'var(--cream-2)'}
              />
              <textarea placeholder="Décrivez votre projet... *" required rows={5} value={form.message}
              ...
              <button type="submit" disabled={status === 'sending'} style={{
                background: status === 'success' ? '#22c55e' : 'var(--gold)',
                color: 'white', padding: '16px', borderRadius: 8,
              ...
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', textAlign: 'center' }}>
                ⚠️ À configurer : remplacez YOUR_FORM_ID dans App.jsx par votre ID Formspree
              </p>
            </div>
          </form>
        </div>
